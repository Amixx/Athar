...
- `athar/matcher_graph.py` — Matching stages for graph diffing. Implements deterministic typed-path propagation from matched root pairs (unique 1:1 buckets only) and scored secondary matching for unresolved non-root entities (deterministic small-block assignment, large-block signature fallback, and explicit tie/margin ambiguity rejection).
